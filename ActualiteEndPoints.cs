...
public class ActualitesController : ControllerBase
      {
            ...
            [HttpGet("last-titles-by-category")]
            public async Task<IActionResult> GetLastTitlesByCategory()
            {
                var result = await _db.Set<Actualite>()
                    ...
                    {
                        Categorie = g.Key,
                        // On récupère le dernier objet complet pour extraire plusieurs propriétés
                        DerniereActu = g.OrderByDescending(a => a.Id)
                                        .Select(a => new { a.Titre, a.DescriptionCourte })
                                        .FirstOrDefault()
                    })
                    .ToListAsync();
